...
import { AVLState, AVLNode, AVLOperation } from '@/types/avlTree';
...
// Helper function to create an AVL Tree step by step
const createStepByStepTree = (numbers: number[]): {operations: AVLOperation[], steps: AVLNode[]} => {
  const avl = new AVLTree();
  const steps: AVLNode[] = [];
  let allOperations: AVLOperation[] = [];

  // For each number, add it to the tree and save intermediate state
  for (let i = 0; i < numbers.length; i++) {
    const ops = avl.insert(numbers[i]);
    allOperations = [...allOperations, ...ops];
    
    // Save a copy of the current tree state after each insertion
    const currentRoot = avl.getRoot();
    if (currentRoot) {
...
const createDeleteStepByStepTree = (tree: AVLNode | null, valueToDelete: number): {operations: AVLOperation[], steps: AVLNode[]} => {
  const avl = new AVLTree();
  if (tree) {
    avl.setRoot(JSON.parse(JSON.stringify(tree))); // Deep clone to avoid modifying original
  }
  
  // Capture initial state
  const initialTree = JSON.parse(JSON.stringify(avl.getRoot()));
  const steps: AVLNode[] = [initialTree];
  
  // Perform delete operation and capture operations
  const operations = avl.delete(valueToDelete);
  
  // Capture final state after deletion
  const finalTree = JSON.parse(JSON.stringify(avl.getRoot()));
...
const avlTreeSlice = createSlice({
  ...
  reducers: {
    insert: (state, action: PayloadAction<number>) => {
      const avl = new AVLTree();
      if (state.tree) {
        avl.setRoot(state.tree);
      }
      const operations = avl.insert(action.payload);
      state.tree = avl.getRoot();
    ...
    search: (state, action: PayloadAction<number>) => {
      ...
      state.operations = avl.search(action.payload);
      state.currentOperation = 0;
      state.isAnimating = true;
      state.error = null;
    },
    ...
    traverse: (state, action: PayloadAction<'INORDER' | 'PREORDER' | 'POSTORDER' | 'LEVELORDER'>) => {
      ...
      switch (action.payload) {
        case 'INORDER':
          state.operations = avl.inorder();
          break;
        case 'PREORDER':
          state.operations = avl.preorder();
          break;
        case 'POSTORDER':
          state.operations = avl.postorder();
          break;
        case 'LEVELORDER':
          state.operations = avl.levelOrder();
          break;
      }
        ...
        if (currentOp && currentOp.type !== 'COMPARE') {
          ...
          switch (currentOp.type) {
            ...
            case 'VISIT':
              ...
              break;
            case 'CHECK_BALANCE':
              if (currentOp.node && Math.abs(currentOp.balanceFactor) > 1) {
                showErrorToast(
                  `Imbalance Detected: ${currentOp.node.value}`, 
                  `Balance factor is ${currentOp.balanceFactor}`
                );
              }
              break;
            case 'ROTATE':
              if (currentOp.node && currentOp.rotationType) {
                const rotationType = currentOp.rotationType === 'LEFT' ? 'Left' : 'Right';
                showSuccessToast(
                  `${rotationType} Rotation: ${currentOp.node.value}`,
                  `Rebalancing tree`
                );
              }
              break;
