...
import { getRandomOddNumber } from '@/helpers/pathFinding/maze';
...
export const  addVerticalBlocks = async(
  grid: CellType[][],
  updateCells: MazeAlgProps['updateCells'],
  stage: number
) => {
...
export const  addHorizontalBlocks = async(
  grid: CellType[][],
  updateCells: MazeAlgProps['updateCells'],
  stage: number
) => {
...
export const getRightBottomCells =({
  rows,
  cols,
  stage,
  right,
...
export const getBottomLeftCells =({
  rows,
  stage,
  bottom,
  left,
...
export const  generateLabyrinth = async({
  rows,
  cols,
  entry,
  exit,
